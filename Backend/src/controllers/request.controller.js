...
async function requestMoney(req, res) {
  try {
    ...
    // 🔥 UPDATED: now 'to' is accountId (NOT userId)
    const toAccount = await accountModel.findById(to);

    if (!toAccount) {
      return res.status(404).json({
        message: "Account not found", // ✅ clearer message
      });
    }

    // 🔥 NEW: get user from account
    const receiver = await userModel.findById(toAccount.user);
    ...
    // 🔥 UPDATED: ensure 'to' matches correct user (from account)
    const existing = await Request.findOne({
      from: req.user._id,
      to: receiver._id, // ✅ use actual userId
      amount,
      status: "pending",
    });
    ...
    // 🔥 UPDATED: store actual userId (not accountId)
    const request = await Request.create({
      from: req.user._id,
      to: receiver._id, // ✅ FIXED
      amount,
    });
    res.status(201).json({
